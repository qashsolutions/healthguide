/**
 * 08-Companionship: Careseeker Rate Visit Screen Tests (Phase 10)
 * Screen: (protected)/careseeker/rate-visit.tsx
 * Elder rates companion after completed visit
 */
...
jest.mock('@/lib/supabase', () => { mockFrom = jest.fn(() => mockChain); return {
  supabase: {
    ...
    auth: {
      ...
      getUser: jest.fn().mockResolvedValue({ data: { user: { id: 'careseeker-1' } } }),
...
jest.mock('@/components/ui/StarRating', () => {
  ...
  return {
    StarRating: ({ rating, onChange, showLabel }: any) =>
      React.createElement(Pressable, { onPress: () => onChange && onChange(4), testID: 'star-rating' },
...
jest.mock('@/theme/colors', () => ({
  colors: {
    ...
    neutral: { 100: '#F3F4F6', 200: '#E5E7EB', 300: '#D1D5DB', 900: '#111827' },
...
import ElderRateVisitScreen from '@/app/(protected)/careseeker/rate-visit';
...
describe('08-Companionship: Careseeker Rate Visit Screen', () => {
  beforeEach(() => {
    jest.clearAllMocks();
    ...
    // First single() = visit data, Second single() = companion profile
    mockSingle
      .mockResolvedValueOnce({ data: mockVisitData, error: null })
      .mockResolvedValueOnce({ data: mockCompanionProfile, error: null });
  });

  // ── Renders ───────────────────────────────────────────────────────────────

  it('renders without crashing', async () => {
    await act(async () => render(<ElderRateVisitScreen />));
    expect(screen.getAllByText(/Rate|rate/i)[0]).toBeTruthy();
  });
  ...
  it('shows visit duration after loading', async () => {
    await act(async () => render(<ElderRateVisitScreen />));
    await waitFor(() => {
      expect(screen.getAllByText(/120|2h|2 hr|2hr/i)[0]).toBeTruthy();
    });
  });
  ...
  it('renders reason textarea', async () => {
    await act(async () => render(<ElderRateVisitScreen />));
    await waitFor(() => {
      expect(screen.getByPlaceholderText(/comment|reason|tell us|feedback|appreciate|better/i)).toBeTruthy();
    });
  });
  ...
  it('renders Submit button', async () => {
    await act(async () => render(<ElderRateVisitScreen />));
    await waitFor(() => {
      expect(screen.getAllByText(/Submit/i)[0]).toBeTruthy();
    });
  });
  ...
  it('renders Skip button', async () => {
    await act(async () => render(<ElderRateVisitScreen />));
    await waitFor(() => {
      expect(screen.getAllByText(/Skip/i)[0]).toBeTruthy();
    });
  });
  ...
  it('selecting star updates rating display', async () => {
    ...
    expect(screen.getAllByText(/4 stars/i)[0]).toBeTruthy();
  });

  it('Submit inserts to visit_ratings table with correct fields', async () => {
    // Mock the visit_ratings insert
    ...
    // Enter reason
    const reasonInput = screen.getByPlaceholderText(/comment|reason|tell us|feedback|appreciate|better/i);
    fireEvent.change(reasonInput, { target: { value: 'Great companion, very kind!' } });
  ...
  it('Skip button navigates without inserting rating', async () => {
    await act(async () => render(<ElderRateVisitScreen />));
    await waitFor(() => screen.getAllByText(/Skip/i)[0]);

    await act(async () => {
      fireEvent.click(screen.getAllByText(/Skip/i)[0]);
    });
  ...
  it('NEGATIVE: shows error when reason is too short (< 10 chars)', async () => {
    ...
    await waitFor(() => {
      expect(screen.getAllByText(/10 char|too short|minimum/i)[0]).toBeTruthy();
    });
  });

  it('NEGATIVE: visit not found shows error state', async () => {
